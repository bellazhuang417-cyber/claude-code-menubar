...
**A. GIF（推荐）** — 每种心情用一个独立的 .gif，动画时序由艺术家在文件里烤好：
...
**B. Sprite sheet** — 一张大图 + 每种心情的行列范围，运行时 CSS `steps()` 播放：
...
```json
{
  ...
  "display": { "w": 108, "h": 116 },
  "animations": {
    "input": { "row": 1, "col": 0, "count": 8, "duration": 0.7 },
    "done":  { "row": 4, "col": 0, "count": 5, "duration": 0.75 }
  }
}
```

`input` = 需要你确认时的心情，`done` = 任务完成庆祝。切皮肤：
